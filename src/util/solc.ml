...
let get_solc () =
  ...
  else if BatString.starts_with !Options.solc_ver "0.4" && not (List.mem !solc_ver solv_lst) then "solc_0.4.25"
  else if List.mem !solc_ver solv_lst then "solc_" ^ !solc_ver (* e.g., solc_0.4.25 *)
  else raise UnsupportedSolc

let rec get_json_ast' file lst =
  match lst with
  | [] -> raise CompilationError
  | solc::tl ->
    let buf = Unix.open_process_in (solc ^ " --ast-compact-json " ^ file ^ " 2>/dev/null") in
    try
    ...
    with e ->
      match Unix.close_process_in buf with
      | WEXITED n when n!=0 -> get_json_ast' file tl
      | _ -> assert false

let get_json_ast file =
  let solc = get_solc () in
  let solcs = if BatString.starts_with solc "solc_0.4.1" then [solc; "solc_0.4.25"] else [solc] in
  get_json_ast' file solcs

let rec is_compilable' file lst =
  match lst with
  | [] -> false
  | solc::tl ->
    let b = Sys.command (solc ^ " " ^ file ^ " 2>/dev/null") = 0 in
    if b then true
    else is_compilable' file tl

let is_compilable : string -> bool
= fun file ->
  let solc = get_solc () in
  let solcs = if BatString.starts_with solc "solc_0.4.1" then [solc; "solc_0.4.25"] else [solc] in
  is_compilable' file solcs
